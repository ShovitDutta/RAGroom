...
const requiredEmbeddingModel = process.env.OLLAMA_EMBEDDING_MODEL || 'nomic-embed-text:latest';
...
async function checkOllama() {
  ...
  try {
    ...
    if (hasChatModel) {
      ...
      console.log(`Please run 'ollama pull ${requiredChatModel}' to download it.`);
    }
    ...
    if (hasEmbeddingModel) {
      ...
      console.log(`Please run 'ollama pull ${requiredEmbeddingModel}' to download it.`);
    }
    ...
    console.error('Please ensure Ollama is running and accessible at http://localhost:11434.');
    process.exit(1);
  }
}
